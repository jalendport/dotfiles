...
A wizard is ephemeral by default — built for one run, saved to `.agents/wizards/<name>.sh`, deleted when the job's done. Move it into the repo's `scripts/` and commit it only when the user wants a repeatable setup path that should live in the repo.
