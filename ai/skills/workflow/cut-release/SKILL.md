...
- **Pull latest first** — the user almost always says "pull the latest before you start." Fetch and pull every branch involved (`main`/`master` *and* `develop` if git-flow).

## 1. Detect the release pattern (don't assume)
Read the repo's own history so you copy it precisely:
- **Branch model** — does `develop` exist and lead `master`/`main`? `develop` + a `master` that only ever moves at releases = **git-flow-lite** (the user's plugin repos). No develop branch / releases straight off main = **trunk**. Note older releases in these repos show a `Merge branch 'develop' for X.Y.Z release` commit — that `--no-ff` pattern is **retired**, don't copy it forward (see step 3).
...
- **Changelog format** — Keep a Changelog with an `## Unreleased` (or `## [Unreleased]`) heading is the user's norm. New features are usually already staged there.
- **Tag format** — `git tag --list | tail`. Tags are **bare semver, no `v` prefix** (`3.1.0`, not `v3.1.0`), as a **lightweight tag**. On git-flow-lite it lands on the `Finalize X.Y.Z release` commit (which `master` fast-forwards onto); older tags sit on retired merge commits — match the version format, not the old placement.
...
3. Commit the finalize with a single-line message per the user's commit convention, e.g. `Bump version to 3.1.0` or `Finalize 3.1.0 release` (+ `Co-Authored-By:` trailer since an agent is committing).
...
- If the repo publishes via GitHub Releases / Packagist / a Craft plugin store, note whether the push already triggers it or whether the user needs a manual step (don't create a GitHub Release unless asked).
